...
impl DiskInode {
    ...
    pub fn increase_size(
        ...
        #[cfg(feature = "journal")]
        let inode_buf = get_buffer_dyn(block_device, disk_inode_block_id as usize).unwrap();
        #[cfg(feature = "journal")]
        handle.get_write_access(&inode_buf).unwrap();
        #[cfg(feature = "journal")]
        handle.dirty_metadata(&inode_buf).unwrap();
        ...
        get_block_cache(self.indirect2 as usize, Rc::clone(block_device)).modify(
            0,
            |indirect2: &mut IndirectBlock| {
                while (a0 < a1) || (a0 == a1 && b0 < b1) {
                    ...
                    #[cfg(feature = "journal")]
                    let buf = get_buffer_dyn(block_device, indirect2[a0] as usize).unwrap();
                    #[cfg(feature = "journal")]
                    handle.get_write_access(&buf).unwrap();

                    // fill current
                    get_block_cache(indirect2[a0] as usize, Rc::clone(block_device)).modify(
                        0,
                        |indirect1: &mut IndirectBlock| {
    ...
    pub fn decrease_size(
        ...
        if current_blocks > INDIRECT1_BOUND as u32 {
            ...
            get_block_cache(self.indirect2 as usize, Rc::clone(block_device)).read(
                0,
                |indirect2: &IndirectBlock| {
                    while (a0 < a1) || (a0 == a1 && b0 < b1) {
                        ...
                        get_block_cache(indirect2[a1] as usize, Rc::clone(block_device)).read(
                            0,
        ...
        if current_blocks > INODE_DIRECT_COUNT as u32 {
            ...
            get_block_cache(self.indirect1 as usize, Rc::clone(block_device)).read(
                0,
                |indirect1: &IndirectBlock| {
        ...
        get_block_cache(self.indirect2 as usize, Rc::clone(block_device)).modify(
            ...
            |indirect2: &mut IndirectBlock| {
                ...
                for entry in indirect2.iter_mut().take(a1) {
                    ...
                    v.push(*entry);
                    get_block_cache(*entry as usize, Rc::clone(block_device)).modify(
                        0,
                        |indirect1: &mut IndirectBlock| {
                ...
                if b1 > 0 {
                    ...
                    v.push(indirect2[a1]);
                    get_block_cache(indirect2[a1] as usize, Rc::clone(block_device)).modify(
                        0,
                        |indirect1: &mut IndirectBlock| {
    ...
    pub fn write_at(
        ...
        block_device: &Rc<dyn BlockDevice>,
        #[cfg(feature = "journal")] handle: &mut jbd::Handle,
        #[cfg(feature = "journal")] metadata: bool,
    ) -> usize {
        let mut start = offset;
        let end = (offset + buf.len()).min(self.size as usize);
        assert!(start <= end);
