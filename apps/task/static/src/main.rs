...
fn main() {
    ...
    for ii in 0..PAYLOAD_KIND {
        ...
        thread::spawn(move || {
            set_priority(datalen as isize);
            set_priority(-(sleeplen as isize));
    ...
    let timeout = WaitQueue::new().wait_timeout(Duration::from_millis(20000));
