    ...
    pub fn epoll_wait(&self, expire_time: usize) -> Vec<EpollEvent> {
        ...
        loop {
            ...
            for req_event in events.iter() {
                if let Some(file) = &inner.fd_manager.fd_table[req_event.data as usize] {
...
impl Read for EpollFile {
    fn read(&mut self, _buf: &mut [u8]) -> AxResult<usize> {
        Err(AxError::Unsupported)
    }
}

impl Write for EpollFile {
...
impl Seek for EpollFile {
    fn seek(&mut self, _pos: SeekFrom) -> AxResult<u64> {
        Err(AxError::Unsupported)
    }
}

impl FileExt for EpollFile {
...
impl FileIO for EpollFile {
    ...
    fn ready_to_read(&mut self) -> bool {
        ...
        for req_event in events.iter() {
            if let Some(file) = inner.fd_manager.fd_table[req_event.data as usize].as_ref() {
                let mut ret_event_type = EpollEventType::empty();
                if file.lock().is_hang_up() {
                    ret_event_type |= EpollEventType::EPOLLHUP;
                }
