...
pub fn syscall_epoll_create1(_flag: usize) -> isize {
    ...
    if let Ok(num) = inner.alloc_fd() {
        inner.fd_manager.fd_table[num] = Some(Arc::new(SpinNoIrq::new(file)));
        num as isize
    } else {
...
pub fn syscall_epoll_ctl(epfd: i32, op: i32, fd: i32, event: *const EpollEvent) -> isize {
    ...
    if inner.fd_manager.fd_table[fd as usize].is_none() {
    ...
    if let Some(file) = inner.fd_manager.fd_table[epfd as usize].as_ref() {
        if let Some(epoll_file) = file.lock().as_any().downcast_ref::<EpollFile>() {
...
pub fn syscall_epoll_wait(
    ...
    let epoll_file = if let Some(file) = inner.fd_manager.fd_table[epfd as usize].as_ref() {
        if let Some(epoll_file) = file.lock().as_any().downcast_ref::<EpollFile>() {
            epoll_file.clone()
        } else {
            return ErrorNo::EBADF as isize;
