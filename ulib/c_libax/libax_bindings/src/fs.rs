...
use libax::{fs::File, io::Read, io::Write, sync::Mutex};
...
/// return 0 if success.
#[no_mangle]
pub unsafe extern "C" fn ax_stat(path: *const c_char, stat: &mut Kstat) -> isize {
    ...
    match libax::fs::File::open(path) {
        Ok(file) => {
            ...
            stat.st_uid = 1000;
            stat.st_gid = 1000;
            stat.st_size = file.metadata().unwrap().len() as u64;
            stat.st_blksize = 512;
...
/// return 0 if success.
#[no_mangle]
pub unsafe extern "C" fn ax_fstat(fd: usize, stat: &mut Kstat) -> isize {
    if fd >= FILE_LIMIT || FD_TABLE.lock()[fd].is_none() {
        return -LinuxError::EBADF.code() as _;
    }
