...
#[doc = r" Proxy"]
pub struct _CNT_OR_UIFCPYW<'a> {
    w: &'a mut W,
}
impl<'a> _CNT_OR_UIFCPYW<'a> {
...
impl R {
    ...
    #[inline]
    pub fn cnt_or_uifcpy(&self) -> CNT_OR_UIFCPYR {
        let bits = {
            const MASK: bool = true;
            const OFFSET: u8 = 31;
...
impl W {
    ...
    #[doc = "Bit 31 - if IUFREMAP=0 than CNT with read write access else UIFCPY with read only access"]
    #[inline]
    pub fn cnt_or_uifcpy(&mut self) -> _CNT_OR_UIFCPYW {
        _CNT_OR_UIFCPYW { w: self }
    }
}
