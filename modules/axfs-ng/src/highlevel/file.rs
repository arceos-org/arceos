...
impl CachedFile {
    ...
    fn write_at_locked(&self, buf: &[u8], offset: u64) -> VfsResult<usize> {
    ...
    pub fn append(&self, buf: &[u8]) -> VfsResult<(usize, u64)> {
...
impl FileBackend {
    ...
    pub fn append(&self, buf: &[u8]) -> VfsResult<(usize, u64)> {
        match self {
            Self::Cached(cached) => cached.append(buf),
            Self::Direct(loc) => loc.entry().as_file()?.append(buf),
        }
    }
